...
def create_precision_comparison_chart():
    """
    Cria um gráfico de barras agrupadas para comparar a precision das ferramentas
    de merge em diferentes níveis de dificuldade.
    """
    
    # Dados de precision organizados
    data = {
    ...
    for i, tool in enumerate(tools):
        ...
        for difficulty in difficulties:
            precision = tool_data[tool_data['Difficulty'] == difficulty]['Precision'].iloc[0]
    ...
    ax.set_title('Comparação de Precision entre Ferramentas de Merge\npor Nível de Dificuldade', 
    ...
    # Adicionar linha de referência em 0.5 (precision média)
    ax.axhline(y=0.5, color='red', linestyle='--', alpha=0.5, linewidth=1)
    ax.text(0.02, 0.52, 'Precision = 0.5', transform=ax.transAxes, 
           fontsize=9, color='red', alpha=0.7)
    
    return fig, ax

def save_chart(filename='precision_comparison.png', dpi=300):
    """
    ...
    """
    fig, ax = create_precision_comparison_chart()
...
def print_data_summary():
    ...
    """
    print("=== Resumo dos Dados de Precision ===")
    ...
    print("\nObservações:")
    print("- JDime apresenta melhor precision em cenários 1->N")
    print("- IntelliMerge se destaca em cenários 1->1")
    print("- FSTMerge tem melhor performance em cenários N->N")

if __name__ == "__main__":
    # Exemplo de uso
    print("Gerando gráfico de comparação de precision...")
    
    # Imprimir resumo dos dados
    print_data_summary()
    
    # Criar e salvar o gráfico
    fig, ax = save_chart('precision_comparison_grayscale.png')
