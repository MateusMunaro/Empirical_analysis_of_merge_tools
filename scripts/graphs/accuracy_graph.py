...
def create_accuracy_comparison_chart():
    """
    Cria um gráfico de barras agrupadas para comparar a accuracy das ferramentas
    de merge em diferentes níveis de dificuldade.
    """
    
    # Dados de accuracy organizados
    data = {
    ...
    for i, tool in enumerate(tools):
        ...
        for difficulty in difficulties:
            accuracy = tool_data[tool_data['Difficulty'] == difficulty]['Accuracy'].iloc[0]
    ...
    ax.set_title('Comparação de Accuracy entre Ferramentas de Merge\npor Nível de Dificuldade', 
    ...
    # Adicionar linha de referência em 0.5 (accuracy média)
    ax.axhline(y=0.5, color='red', linestyle='--', alpha=0.5, linewidth=1)
    ax.text(0.02, 0.52, 'Accuracy = 0.5', transform=ax.transAxes, 
           fontsize=9, color='red', alpha=0.7)
    
    return fig, ax

def save_chart(filename='accuracy_comparison.png', dpi=300):
    """
    ...
    """
    fig, ax = create_accuracy_comparison_chart()
...
def print_data_summary():
    ...
    """
    print("=== Resumo dos Dados de Accuracy ===")
