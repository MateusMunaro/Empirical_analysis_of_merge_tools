...
def create_f1_score_comparison_chart():
    """
    Cria um gráfico de barras agrupadas para comparar o F1-Score das ferramentas
    de merge em diferentes níveis de dificuldade.
    """
    
    # Dados de f1_score organizados
    data = {
    ...
    for i, tool in enumerate(tools):
        ...
        for difficulty in difficulties:
            f1_score = tool_data[tool_data['Difficulty'] == difficulty]['F1_Score'].iloc[0]
    ...
    ax.set_title('Comparação de F1-Score entre Ferramentas de Merge\npor Nível de Dificuldade', 
    ...
    # Adicionar linha de referência em 0.5 (f1-score médio)
    ax.axhline(y=0.5, color='red', linestyle='--', alpha=0.5, linewidth=1)
    ax.text(0.02, 0.52, 'F1-Score = 0.5', transform=ax.transAxes, 
           fontsize=9, color='red', alpha=0.7)
    
    return fig, ax

def save_chart(filename='f1_score_comparison.png', dpi=300):
    """
    ...
    """
    fig, ax = create_f1_score_comparison_chart()
...
def print_data_summary():
    ...
    """
    print("=== Resumo dos Dados de F1-Score ===")
